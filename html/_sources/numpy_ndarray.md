...
Since a list has only one dimension, then the resulting array also has one dimension. We can get the size (shape) of an array using its {{ndarray_shape}} property:
...
It is very common to generate unidimensional arrays of equally spaced values, such as `[0, 1, 2, 3, 4]` or `[0, 0.1, 0.2, 0.3]`. One way would be to use a series of functions such as:

```{code-cell} ipython3
np.array(list(range(5)))
```

which creates a range from 0 (incl.) to 5 (excl.) using `range`, then converts this range to a list, then creates an array from this list. Hopefully, NumPy provides a more efficient shortcut for it: {{np_arange}}

```{code-cell} ipython3
np.arange(5)
```

This function takes the same arguments as the Python's [range](python_for_range.md) function, and directly creates an array from this range.

:::{good-practice} np.arange
The arguments of `np.arange` can be not only integers but also floats. For example:
...
However, this practice is not recommended due to potential floating point problems that can generate surprizing results. Without going further into these floating point issues, just remind that in most cases, `np.arange` should be used only with integers. For the example above:

```
np.arange(5) / 10
```

is completely safe and also generates:
...
Another function to generate To create equally spaced series of float is {{np_linspace}}, which takes as arguments the initial value, the final value, and the number of points in the new array. By default, `np.linspace` includes both the initial and final values. For example, to create an array that goes from 5.0 (included) to 10.0 (included) and that contains 11 elements:
